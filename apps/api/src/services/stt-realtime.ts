...
export type SttRealtimeOpts = {
  realtimeModel?: string;      // e.g. "gpt-4o-realtime-preview"
  sttModel?: string;           // e.g. "gpt-4o-mini-transcribe" or "whisper-1"
  language?: string;           // "en" | "es" | ...
  silenceMs?: number;          // server VAD
  partialCommitMs?: number;    // how often to TRY partial commits (ms)
  minCommitMs?: number;        // >=100ms; we’ll default to 200ms for safety
};

export class SttRealtimeSession {
  private oai?: WebSocket;
  private oaiReady = false;

  private opusIn?: PassThrough;
  private ff?: ChildProcessWithoutNullStreams;
  private pcmOut?: PassThrough;

  private partialTimer?: ReturnType<typeof setInterval>;
  private currentTurnAccum = "";
  private sessionIdStr = "";

  // Guard state (server-visible)
  private bytesSinceCommit = 0;
  private minCommitBytes = 0;
  private gotPcmSinceLastCommit = false;
  private commitInFlight = false; // <- NEW: prevents double-commit during VAD races

  // 16kHz mono, s16le: 2 bytes/sample => 32,000 bytes/sec => 32 bytes/ms
  private static readonly PCM_BYTES_PER_MS = 32;

  constructor(
    private apiKey: string,
    private sendToClient: (msg: any) => void,
    private opts: SttRealtimeOpts = {}
  ) {}

  private startPartialTimer() {
  ...
  private stopPartialTimer() {
    try { if (this.partialTimer) clearInterval(this.partialTimer); } catch {}
    this.partialTimer = undefined;
  }

  /** Only commit when fresh PCM >= threshold and no commit is in flight */
  private commitIfReady() {
    if (!this.oaiReady) return;
    if (this.commitInFlight) return;                 // wait for server ACK
    if (!this.gotPcmSinceLastCommit) return;         // nothing new
    if (this.bytesSinceCommit < this.minCommitBytes) return; // < threshold
    try {
  ...
  start(sessionId: string) {
    ...
    this.pcmOut.on("data", (pcmChunk: Buffer) => {
      if (!this.oaiReady) return;
  ...
  appendOpusBase64(b64: string) {
    if (!this.opusIn) return;
  ...
  close() {
    this.stopPartialTimer();
    try { this.opusIn?.destroy(); } catch {}
    try { this.pcmOut?.destroy(); } catch {}
    try { this.ff?.kill("SIGKILL"); } catch {}
    try { this.oai?.close(); } catch {}
  }
}
